...
# Usage: ./make_sge_RNAseq_old.sh

cpu=4
index=hg19
inputdir=$HOME/nearline/BU/fastq/
...
for i in *{MyersLab,JiangFanChen}*.fastq;
do
    R1=$i
    samplename=${R1/.fastq/}
    samplename=${samplename/*[MyersLab|JiangFanChen]_/}
    samplename=${samplename/_*/}

    # Ct_B6341,Ct_B3732,Ct_B6096,Ct_B6124,Ct_B6356,
    # HD_B4242,HD_B3584,HD_B4430,HD_B4189,HD_B4412
    if [[ "$samplename" =~ "6341|3732|6096|6124|6356" ]]; then
        let c=$c+20
        color=$c",255,0"
        samplename="BU5vs5_Ct_$samplename"
    else
        let h=$h+20
        color="255,"$h",51"
        samplename="BU5vs5_HD_$samplename"
    fi
    ...
    #tophat -o $outputdir/$samplename --no-convert-bam -p $cpu --read-mismatches $mm $tophat $strandoption --min-anchor-length 8 --min-intron-length 30 --max-intron-length 50000 --splice-mismatches 1 --max-multihits 100 --no-coverage-search genome_offrate3 $R1 $R2
    #
    ############################################
    ...
    #cufflinks -q --no-update-check $strandoption -o ./ -p $cpu -g \$ANNOTATION/gencode.v14.annotation.gtf -M \$ANNOTATION/chrM.rRNA.tRNA.gtf -b \$BOWTIE_INDEXES/genome.fa --multi-read-correct accepted_hits.sam
    cufflinks -q --no-update-check $strandoption -o ./ -p $cpu -g \$ANNOTATION/genes.gtf -M \$ANNOTATION/chrM.rRNA.tRNA.gtf -b \$BOWTIE_INDEXES/genome.fa --multi-read-correct accepted_hits.sam
...
# run cuffdiff
cuffdiffoutput=$outputdir/HD_cuffdiff_5vs5
[ -d $cuffdiffoutput ] || mkdir -p $cuffdiffoutput
...
" | sed 's/^[ \t]*//g'  > $cuffdiffoutput/submit.sge

cat $cuffdiffoutput/submit.sge

jobid=`qsub $cuffdiffoutput/submit.sge | cut -f3 -d' '`
ln -fs $HOME/sge_jobs_output/sge_job.$jobid.out $cuffdiffoutput/sge.log

echo "
Your job is submitted (jobID: $jobid) !
The SGE script is saved as $cuffdiffoutput/submit.sge
The running log is saved as $cuffdiffoutput/sge.log
"
