...
proximity=ifelse(abs(distance_peakcenter2TSS)<=1000, "proximal", "distal")

#> table(uniqueness,proximity)
#          proximity
...
peaks=cbind(id=paste(chr,start_peak,end_peak, sep="_"), peaks, HD.lognorm=y, CT.lognorm=x.norm, uniqueness=uniqueness, proximity=proximity)
write.table(peaks, "~/Dropbox/huntington_bu/data/h3k4me3_peaks_union.signal.neighborhood.annotated.bed.v4.xls", quote =F, sep="\t", row.names =F, col.names =T)

################################################
## normalization before vs. after
################################################

pdf("~/Dropbox/huntington_bu/figs/Fig2A.peak_signal_HD.vs.Ct.pdf", width=7, height=7)
## XY plot
par(mfrow=c(2,2))
...
pdf("~/Dropbox/huntington_bu/figs/Fig1B.peak_dize_distribution.pdf", width=6, height=4)
...
################################################
pdf("~/Dropbox/huntington_bu/figs/Fig1C.distance_peakcenter2TSS.pdf", width=6, height=4)
...
################################################
pdf("~/Dropbox/huntington_bu/figs/Fig1D.peak_vs_sampleCount.pdf", width=6, height=6)
require('ggplot2')
...
pdf("~/Dropbox/huntington_bu/figs/Fig1E.peakCount.btw.HD.CT.pdf", width=6, height=6)
...
################################################
# the following figure is to show that using 3 samples as cutoff to define unique peaks is as good as using a foldchange cutoff, except some outliers.
################################################
pdf("~/Dropbox/huntington_bu/figs/FigS2.uniqness.foldchange.samplecount.pdf", width=7, height=6)
